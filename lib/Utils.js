        ...
        await test.step("🛡️ JSON Schema Validation", async () => {
            ...
            if (!valid) {
                const errorMsg = validate.errors.map(err => 
                    `Path: ${err.instancePath} | Message: ${err.message} | Data: ${JSON.stringify(err.data)}`
                ).join('\n');
                
                console.error("[Schema Error] Validation failed:\n", errorMsg);
                expect(valid, `JSON Schema validation failed:\n${errorMsg}`).toBe(true);
            } else {
