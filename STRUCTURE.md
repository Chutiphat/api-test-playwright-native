...
### **วิธีการเลือกบางเคส (How to select cases):**
เมื่อกดปุ่ม **"Build with Parameters"** ใน Jenkins คุณจะพบหัวข้อ **TEST_SCOPE** ให้เลือก:
1.  **ALL**: รันไฟล์เทสทั้งหมดในโฟลเดอร์ `tests/`
2.  **Scene 1 Only**: รันเฉพาะไฟล์ `tests/test02-scene1.spec.js`
3.  **Scene 2 Only**: รันเฉพาะไฟล์ `tests/test02-scene2.spec.js`

> **💡 วิธีเพิ่ม Scope ใหม่:** หากคุณสร้างไฟล์เทสใหม่ (เช่น `withdraw.spec.js`) และต้องการให้เลือกได้ใน Jenkins คุณต้องไปเพิ่มชื่อไฟล์นั้นในไฟล์ `Jenkinsfile` ภายใต้หัวข้อ `parameters` และ `Run Tests` stage ครับ
